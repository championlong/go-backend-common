...
var (
	ErrClosed      = errors.New("hbase: database is closed")
...
type Options struct {
	Dialer  func(context.Context) (thrift.TTransport, error)
	Factory func(context.Context) thrift.TProtocolFactory
	OnClose func(*Conn) error

	ThriftFactory      thrift.TProtocolFactory
	PoolSize           int
	MinIdleConns       int
...
func (p *ConnPool) dialConn(c context.Context, pooled bool) (*Conn, error) {
	...
	factory := p.opt.Factory(c)

	cn := NewConn(socketConn, factory)
	cn.pooled = pooled
	return cn, nil
}
...
func (p *ConnPool) reaper(frequency time.Duration) {
	...
	for range ticker.C {
		...
		if err != nil {
			slog.Errorf("ReapStaleConns failed: %s", err)
			continue
		}
